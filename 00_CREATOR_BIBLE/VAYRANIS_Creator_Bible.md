...
```mermaid
flowchart LR
    Family["Kutumba (Family Unit)"] --> GramaCouncil["Grama Mandala (Village Council)"]
    GramaCouncil --> TownCouncil["Nagara Mandala (Town Council)"]
    TownCouncil --> CityCouncil["Mahanagara Mandala (City Council)"]
    CityCouncil --> ProvinceCouncil["Pradesha Mandala (Provincial Council)"]
    ProvinceCouncil --> KingdomCourt["Rajya Sabha (Kingdom Court)"]
    KingdomCourt --> ImperialCouncil["Samrajya Mandala (Imperial Council)"]
```
...
```mermaid
erDiagram
    GRAMA_MANDALA ||--|{ NAGARA_MANDALA : sends_delegates
    NAGARA_MANDALA ||--|{ SAMRAJYA_MANDALA : reports_to
    SAMRAJYA_MANDALA ||--|{ MINISTRIES_MANDALAS : oversees
    MINISTRIES_MANDALAS ||--|{ VIDYASHALA : funds
    MINISTRIES_MANDALAS ||--|{ GRANTHALAYA : archives_in
    MINISTRIES_MANDALAS ||--|{ RAKSHA_SENA : directs_defense
    RAKSHA_SENA ||--|{ FOUR_COMMANDS : structures
    RAKSHA_SENA ||--o{ SAHAJEEVAS : partners_with
```
...
```mermaid
flowchart TD
    RS["Raksha Sena (Defense Force)"] --> EC["Earth Command (Bhoomi Sena)"]
    RS --> WC["Water Command (Jala Sena)"]
    RS --> SC["Sky Command (Vayu Sena)"]
    RS --> WildC["Wild Command (Aranya Sena)"]

    EC --> EC_Focus["Infantry, Fortresses, Tunneling, Engineering\nAnimals: Elephants, Yaks, Stone Bears"]
    WC --> WC_Focus["Naval Patrol, Coastal Defense, River Rescue\nAnimals: Sea Turtles, Dolphins"]
    SC --> SC_Focus["Recon, Message Transport, Aerial Medevac\nAnimals: Giant Hawks, Falcons"]
    WildC --> WildC_Focus["Forest Rangers, Border Watch, Monster Containment\nAnimals: Wolves, Hounds, Owls"]
```
...
gantt
    dateFormat  YYYY
    axisFormat %Y
    title Bhuraya Historical Chronology
    section Eras
    Era I - Survival     :e1, 0000, 0500d
    Era II - Settlement   :e2, 0500, 0500d
    Era III - Kingdoms   :e3, 1000, 1000d
    Era IV - Knowledge   :e4, 2000, 1000d
    Era V - Innovation   :e5, 3000, 1000d
    Era VI - Samanvaya   :e6, 4000, 1000d
```
